...
export class StorageComponent implements OnInit {
  ...
  codeModel: CodeModel = {
    language: 'markdown',
    uri: 'markdown.json',
    value: `Ningún documento seleccionado`,
  };
  ...
  loadDocuments() : void {
    ...
    this.setTextInCodeEditor("Ningún documento seleccionado");

    this.documentService.getDocumentsByCollectionId(1).subscribe(
      documents => {
  ...
  generateText() : void {
    ...
    for (let chunk of this.documentChunks) {
      if (chunk.loaded) text += chunk.content;
      else text += chunk.splitFile;
  ...
  setTextInCodeEditor(text: string) : void {
